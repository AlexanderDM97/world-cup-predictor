...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (!predicted_champion) return res.status(400).json({ error: 'Please select your World Cup winner prediction' });

    const db   = getDB();
    const hash = await bcrypt.hash(String(pin), 10);
    const result = db.prepare(
      'INSERT INTO participants (name, pin_hash, predicted_champion) VALUES (?, ?, ?)'
    ).run(String(name).trim(), hash, predicted_champion);
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    if (!name || !pin) return res.status(400).json({ error: 'Name and PIN required' });
    const db          = getDB();
    const participant = db.prepare('SELECT * FROM participants WHERE name = ? COLLATE NOCASE').get(String(name).trim());
